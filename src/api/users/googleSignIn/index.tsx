    ...
    if (findUser) {
        ...
        if (findUser.isRegistered) {
            ...
            const stmt2 = db.prepare(
                `
                    UPDATE users
                    SET firstName = :firstName, lastName = :lastName, avatarUrl = :avatarUrl, verificationCode = :verificationCode, verificationCodeExpiryTime = :verificationCodeExpiryTime, isRegistered = :isRegistered, accountType = :accountType, username = :username, commentsLeft = :commentsLeft
                    WHERE email = :email
                `
            )
        ...
        const stmt = db.prepare(
            `
                INSERT INTO users (_id, email, firstName, lastName, dateCreated, avatarUrl, password, isRegistered, verificationCode, verificationCodeExpiryTime, accountType, username, commentsLeft)
                VALUES (:_id, :email, :firstName, :lastName, :dateCreated, :avatarUrl, :password, :isRegistered, :verificationCode, :verificationCodeExpiryTime, :accountType, :username, :commentsLeft)
            `
        )
